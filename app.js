...
/* ── 1. Storage layer ───────────────────────────────────────────────────── */
// ─── UNIVERSAL CLOUD AUTO-SYNC PERSISTENCE LAYER ───
window._pendingFirebaseChanges = {};

function getActiveUID() {
  if (window.currentUserUID) return window.currentUserUID;
  if (window.FirebaseAuth && window.FirebaseAuth.auth && window.FirebaseAuth.auth.currentUser) {
    return window.FirebaseAuth.auth.currentUser.uid;
  }
  let localUid = localStorage.getItem('resell_firebase_uid');
  if (!localUid) {
    localUid = 'user_' + Math.random().toString(36).substring(2, 11) + '_' + Date.now();
    localStorage.setItem('resell_firebase_uid', localUid);
  }
  return localUid;
}

window.syncChangeToFirebase = async (key, value) => {
  const uid = getActiveUID();
  if (!window.db) {
    window._pendingFirebaseChanges[key] = value;
    console.log(`[Firebase Auto-Sync] Queued '${key}' for UID: ${uid} (waiting for Firestore initialization)`);
    return;
  }
  ...
  try {
    ...
    // Provide standardized keys in the user document
    if (key === 'quotes') payload.savedQuotes = value;
    if (key === 'orders') payload.orders = value;
    if (key === 'addresses') payload.addresses = value;
    if (key === 'user') payload.profile = value;
    if (key === 'cartItems') payload.cartItems = value;
    if (key === 'messages') payload.advisorChat = value;

    // Save to users/{uid}
    await window.db.collection('users').doc(uid).set(payload, { merge: true });

    // Also mirror specific tables for easy Firebase Console inspection
    if (key === 'cartItems') {
      await window.db.collection('carts').doc(uid).set({
        uid,
        cartItems: value,
        cartCount: Array.isArray(value) ? value.length : 0,
        lastUpdated: new Date().toISOString()
      }, { merge: true });
    }
    if (key === 'orders') {
      await window.db.collection('orders_activity').doc(uid).set({
        uid,
        orders: value,
        lastUpdated: new Date().toISOString()
      }, { merge: true });
    ...
    console.log(`✓ [Firebase Auto-Sync] Saved '${key}' to Firestore for UID: ${uid}`, value);
  } catch (err) {
    console.warn(`[Firebase Auto-Sync] Error saving '${key}' to Firestore:`, err);
  }
};
...
window.DB = {
  ...
  set: (k, v) => {
    ...
    // Automatically synchronize EVERY change a user makes directly to Firebase Firestore
    if (typeof window.syncChangeToFirebase === 'function') {
      window.syncChangeToFirebase(k, v);
    }
  }
};

/* ── Activity clock ──────────────────────────────────────────────────────
   One source of truth for "is anyone actually doing anything". Every
   decorative loop on the page subscribes to it and ends its own rAF chain
   when the clock goes quiet, rather than each keeping a permanent loop of
   its own and deciding per frame that there is nothing to do. A frame that
   is never scheduled costs nothing at all; a frame that is scheduled and
   returns early still costs a full render pipeline run, because the browser
   has no way to know the callback did nothing.

   The practical effect is that an unattended tab renders zero frames a
   second instead of one per vsync. Any real input restores it within a
   frame, so nothing is lost but the animation nobody was watching. */
window.Activity = (function () {
  const IDLE_MS = 3000;
  let until = 0, timer = 0;
  const subs = [];
  /* The JS loops can only stop themselves. The looping CSS decor cannot —
     an `animation: ... infinite` runs the render pipeline every vsync for
     the life of the page whatever script does — so going idle also parks
     it, and waking unparks it. This is the half that actually decides what
     an unattended tab costs. */
  function goIdle() { document.body.classList.add('idle'); }

  /* A second, much shorter channel, for the things that should track the
     page moving rather than the reader being present. Scroll events fire
     continuously through a gesture and through momentum, so "still" is just
     a short grace period after the last one — long enough to bridge the gaps
     between events, short enough that stopping reads as immediate. */
  const SCROLL_STOP_MS = 150;
  let scrollUntil = 0;
  const scrollSubs = [];
  function pokeScroll() {
    const wasStill = performance.now() > scrollUntil;
    scrollUntil = performance.now() + SCROLL_STOP_MS;
    if (wasStill) for (let i = 0; i < scrollSubs.length; i++) { try { scrollSubs[i](); } catch (e) {} }
  }
  window.addEventListener('scroll', pokeScroll, { passive: true });

  function poke() {
    const wasIdle = performance.now() > until;
    until = performance.now() + IDLE_MS;
    clearTimeout(timer);
    timer = setTimeout(goIdle, IDLE_MS);
    if (wasIdle) {
      document.body.classList.remove('idle');
      for (let i = 0; i < subs.length; i++) { try { subs[i](); } catch (e) {} }
    }
  }
  ['pointermove','pointerdown','wheel','keydown','touchstart','touchmove','scroll','resize']
    .forEach(t => window.addEventListener(t, poke, { passive: true }));
  document.addEventListener('visibilitychange', () => { if (!document.hidden) poke(); });
  poke();
  return {
    active: () => !document.hidden && performance.now() <= until,
    onWake: f => subs.push(f),
    poke: poke,
    /* True only while the page is actually moving. */
    scrolling: () => !document.hidden && performance.now() <= scrollUntil,
    onScrollStart: f => scrollSubs.push(f)
  };
})();

/* ── Low-end gate ────────────────────────────────────────────────────────
   Measured, the desktop decor costs about the same per frame whatever it is
   running on — which a fast machine absorbs and a slow one does not. Rather
   than degrade everything for everybody, a machine that reports four cores
   or less (or 4GB or less) is told to skip the heaviest layers outright:
   the glyph ribbon does not initialise, and the canopy holds a still
   gradient instead of panning one. Both are pure decoration; nothing about
   the page's meaning depends on either. */
(function () {
  const cores = navigator.hardwareConcurrency || 8;
  const mem   = navigator.deviceMemory || 8;
  if (cores <= 4 || mem <= 4) document.documentElement.classList.add('lowfi');
})();
...
// ─── TIERED BASE PRICING DICTIONARY & COMPONENT DECOMPOSITION ───
// Flagship Tier: Rs. 80,000 to Rs. 1,30,000 (iPhone Pro models, Samsung Ultra)
// Mid-Range Tier: Rs. 30,000 to Rs. 50,000 (OnePlus, Pixel, standard Samsung/Oppo/Vivo)
// Budget / Legacy Tier: Rs. 5,000 to Rs. 15,000 (Realme C series, vintage 10-year devices)
const MODEL_MSRP = {
  // Flagship Tier (Rs. 80,000 to Rs. 1,30,000)
  'iPhone 16 Pro Max (256GB)': 130000,
  'iPhone 16 Pro (256GB)': 129900,
  'iPhone 16 Pro (128GB)': 119900,
  'iPhone 16 (128GB)': 79900,
  'iPhone 15 Pro (128GB)': 124900,
  'iPhone 15 Plus (128GB)': 89900,
  'iPhone 15 (128GB)': 79900,
  'iPhone 14 Pro Max (256GB)': 128900,
  'iPhone 14 Plus (128GB)': 79900,
  'iPhone 14 (128GB)': 69900,
  'iPhone 13 Pro (256GB)': 109900,
  'Galaxy Z Fold 6 (256GB)': 130000,
  'Galaxy Z Flip 6 (256GB)': 99999,
  'Galaxy S24 Ultra (256GB)': 129999,
  'Galaxy S24+ (256GB)': 99999,
  'Galaxy S23+ (256GB)': 84999,
  'Galaxy S22 Ultra (256GB)': 89999,
  'Pixel 9 Pro XL (128GB)': 124999,
  'Pixel Fold (256GB)': 130000,
  'Pixel 8 Pro (128GB)': 106999,
  'OnePlus Open (512GB)': 130000,
  'Xiaomi 14 Ultra (256GB)': 99999,
  'OPPO Find X7 Ultra': 89999,
  'Vivo X100 Pro (256GB)': 89999,
  'Moto Razr 50 Ultra (256GB)': 99999,

  // Mid-Range Tier (Rs. 30,000 to Rs. 50,000)
  'iPhone 13 (128GB)': 49900,
  'iPhone 12 (64GB)': 39900,
  'iPhone 12 mini (64GB)': 34900,
  'iPhone 11 (64GB)': 31900,
  'iPhone SE (3rd Gen) (64GB)': 32900,
  'iPhone XS Max (256GB)': 38000,
  'iPhone X (256GB)': 30000,
  'Galaxy S24 (128GB)': 49999,
  'Galaxy S23 (128GB)': 48999,
  'Galaxy S21 FE (128GB)': 39999,
  'Galaxy A55 5G (128GB)': 39999,
  'Galaxy A35 5G (128GB)': 30999,
  'Galaxy S20 (128GB)': 32000,
  'Pixel 9 (128GB)': 49999,
  'Pixel 8 (128GB)': 49999,
  'Pixel 8a (128GB)': 48999,
  'Pixel 7 Pro (128GB)': 49999,
  'Pixel 7a (128GB)': 43999,
  'Pixel 6 (128GB)': 32000,
  'OnePlus 12 (256GB)': 49999,
  'OnePlus 12R (128GB)': 39999,
  'OnePlus 11 5G (128GB)': 49999,
  'OnePlus 10T 5G (128GB)': 44999,
  'OnePlus Nord 4 (128GB)': 32999,
  'OnePlus 9 Pro (128GB)': 38000,
  'Xiaomi 14 (128GB)': 49999,
  'Xiaomi 13 Pro (256GB)': 49999,
  'POCO F6 5G (256GB)': 31999,
  'Redmi Note 13 Pro+ (256GB)': 31999,
  'Realme GT 6 (256GB)': 40999,
  'Realme GT 5 Pro (256GB)': 42999,
  'Realme GT Neo 3 (256GB)': 36999,
  'Vivo X100 (256GB)': 49999,
  'iQOO 12 (256GB)': 49999,
  'iQOO Neo 9 Pro (256GB)': 35999,
  'Vivo V29 Pro (256GB)': 39999,
  'Vivo V27 Pro (128GB)': 32999,
  'OPPO Reno 12 Pro (256GB)': 36999,
  'OPPO Reno 11 Pro (256GB)': 39999,
  'OPPO Reno 10 Pro (256GB)': 34999,
  'Moto Edge 50 Pro (256GB)': 35999,
  'Moto Edge 40 Pro (256GB)': 49999,
  'Nothing Phone (2) (256GB)': 44999,
  'Nokia X30 (128GB)': 36999,

  // Budget / Legacy Tier (Rs. 5,000 to Rs. 15,000)
  'Realme C55 (128GB)': 11999,
  'Realme C53 (64GB)': 9999,
  'Realme 12 5G (128GB)': 14999,
  'Realme Narzo 60 (128GB)': 14999,
  'Realme 10 Pro+ (128GB)': 14999,
  'Realme 11 Pro+ (256GB)': 14999,
  'Realme P1 Pro (128GB)': 14999,
  'Realme 9 Pro+ (128GB)': 13999,
  'Realme 8 Pro (128GB)': 11999,
  'Realme 7 Pro (128GB)': 9999,
  'Realme 6 Pro (128GB)': 8999,
  'Realme 1 (2018 Legacy)': 5000,
  'Redmi 13C 5G (128GB)': 10999,
  'POCO F5 (128GB)': 14999,
  'POCO X5 Pro (256GB)': 13999,
  'Redmi Note 13 Pro (128GB)': 14999,
  'Redmi Note 12 Pro (128GB)': 13999,
  'Redmi Note 8 Pro (64GB)': 8000,
  'Redmi Note 4 [2016 Vintage]': 5500,
  'Mi 3 [2014 Legacy]': 5000,
  'Galaxy M34 5G (128GB)': 14999,
  'Galaxy S10+ (128GB)': 14999,
  'Galaxy S10 (128GB)': 12999,
  'Galaxy S9 (128GB)': 9999,
  'Galaxy S8 (64GB)': 8000,
  'Galaxy S7 Edge (32GB)': 6500,
  'Galaxy S6 (32GB) [2015 Legacy]': 5500,
  'Galaxy S5 (16GB) [2014 Legacy]': 5000,
  'Galaxy S4 (16GB) [2013 Vintage]': 5000,
  'Galaxy S2 (16GB) [2011 Vintage]': 5000,
  'iPhone 8 (64GB)': 14999,
  'iPhone 7 (32GB)': 11999,
  'iPhone 6s (32GB)': 9999,
  'iPhone 6 (16GB) [2014 Vintage]': 7500,
  'iPhone 5s (16GB) [2013 Legacy]': 6000,
  'iPhone 4S (8GB) [2011 Vintage]': 5000,
  'Pixel 5 (128GB)': 14999,
  'Pixel 4a (128GB)': 12999,
  'Pixel 3 (64GB) [Vintage]': 9000,
  'Pixel 2 XL (128GB) [Legacy]': 7500,
  'Pixel (2016 Original)': 6000,
  'Nexus 6P [2015 Legacy]': 5500,
  'Nexus 5 (16GB) [2013 Legacy]': 5000,
  'OnePlus 8T (128GB)': 14999,
  'OnePlus 7T (128GB)': 12999,
  'OnePlus 6T (128GB)': 10999,
  'OnePlus 5T (64GB) [Vintage]': 8000,
  'OnePlus 3T (64GB) [Legacy]': 6000,
  'OnePlus One (64GB) [2014 Flagship Killer]': 5000,
  'OnePlus Nord CE 4 (128GB)': 14999,
  'Vivo T3 5G (128GB)': 14999,
  'Vivo Y21 (64GB)': 9999,
  'Vivo X5 Max [2014 Legacy]': 5000,
  'OPPO F25 Pro (128GB)': 14999,
  'OPPO F23 (128GB)': 12999,
  'OPPO F17 Pro (128GB)': 9999,
  'OPPO Find 7 [2014 Legacy]': 5000,
  'Moto G84 (256GB)': 14999,
  'Moto G73 (128GB)': 13999,
  'Moto G64 5G (128GB)': 12999,
  'Moto G54 (128GB)': 11999,
  'Moto G1 [2013 Legacy]': 5000,
  'Moto X (2013 Original)': 5000,
  'Nothing Phone (2a) (128GB)': 14999,
  'Nothing Phone (1) (128GB)': 14999,
  'CMF Phone 1 (128GB)': 14999,
  'Nokia G60 (128GB)': 14999,
  'Nokia Lumia 1020 [41MP 2013 Vintage]': 8000,
  'Nokia N9 [2011 Collector]': 6000
};

// Component-level BOM weights summing to exactly 100%
const COMPONENT_WEIGHTS = {
  logic: 0.38,   // Logic board: 38%
  display: 0.28, // Display panel: 28%
  camera: 0.18,  // Camera module: 18%
  battery: 0.08, // Battery cell: 8%
  chassis: 0.08  // Chassis & frame: 8%
};

function getTierDecayParams(msrp) {
  if (msrp >= 80000) return { decay: 0.012, retention: 0.78 }; // Flagship: Rs. 80,000 - Rs. 1,30,000
  if (msrp >= 30000) return { decay: 0.013, retention: 0.76 }; // Mid-range: Rs. 30,000 - Rs. 50,000
  return { decay: 0.014, retention: 0.74 };                    // Budget: Rs. 5,000 - Rs. 15,000
}

function getModelBOMBase(model, brand) {
  const msrp = MODEL_MSRP[model] || (brand === 'apple' ? 85000 : (brand === 'realme' ? 12000 : 35000));
  const { decay } = getTierDecayParams(msrp);
  return {
    msrp: msrp,
    decay: decay,
    logic: Math.round(msrp * 0.38),
    display: Math.round(msrp * 0.28),
    camera: Math.round(msrp * 0.18),
    battery: Math.round(msrp * 0.08),
    chassis: Math.round(msrp * 0.08)
  };
}
...
const STORE_ITEMS = [
  ...
  { id: 18, type: 'dev', brand: 'apple', series: 'flagship', title: 'MacBook Air 15" M2 (2023)', sub: 'Midnight · 16GB/512GB · Batt 97%', grade: 'A+', price: 79999, orig: 134900, icon: 'fa-solid fa-laptop' },
  { id: 19, type: 'dev', brand: 'dell', series: 'flagship', title: 'Dell XPS 15 (RTX 4070)', sub: 'Platinum · 32GB/1TB OLED · Batt 98%', grade: 'A+', price: 129999, orig: 189990, icon: 'fa-solid fa-laptop' },
  { id: 20, type: 'dev', brand: 'lenovo', series: 'flagship', title: 'ThinkPad X1 Carbon Gen 11', sub: 'Carbon Black · 16GB/512GB · Batt 95%', grade: 'A+', price: 74999, orig: 135000, icon: 'fa-solid fa-laptop' },
  ...
  { id: 105, type: 'parts', brand: 'apple', series: 'parts', title: 'MacBook Pro Liquid Retina XDR Panel', sub: '14.2" 120Hz ProMotion · Tested', grade: 'OEM', price: 18500, orig: 28000, icon: 'fa-solid fa-display' },
  { id: 106, type: 'parts', brand: 'dell', series: 'parts', title: 'Dell XPS 86Wh 6-Cell Battery Pack', sub: 'Tested >92% Capacity · OEM', grade: 'OEM', price: 4200, orig: 7500, icon: 'fa-solid fa-battery-three-quarters' }
];
...
function nav(p) {
  ...
  // Requirement: Reset the Exploded hardware stage 3D models to default whenever entering or exiting sell & disassemble page
  if (p === 'quote' || prevPage === 'quote') {
    resetSellAndDisassembleToDefault();
  }
  ...
  /* The ribbon only draws while the page is scrolling now, and a view switch
     changes which canopies exist and where they sit. The scrollTo below
     usually wakes it as a side effect, but not when the page is already at
     the top — so ask directly, once the new view has been laid out. */
  setTimeout(() => { if (window.glyphRepaint) window.glyphRepaint(); }, 60);
  ...
  if (p === 'quote') { setTimeout(updateModels, 50); setTimeout(function() { if (typeof renderLaptopGradeCards === 'function') renderLaptopGradeCards(); }, 80); }
...
function setHeroSample(type) {
  const card = document.getElementById('hero-sample-card');
  const btnPhone = document.getElementById('hero-sample-phone-btn');
  const btnLaptop = document.getElementById('hero-sample-laptop-btn');
  if (!card) return;
  if (type === 'laptop') {
    card.innerHTML = `
      <div class="flex items-start justify-between gap-4 mb-3">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[.16em] text-khaki-300">Sample Laptop Payout</p>
          <p class="text-xs text-slate-300 mt-0.5">Dell XPS 15 (RTX 4070) · 32GB/1TB · Good</p>
        </div>
        <i class="fa-solid fa-laptop text-2xl text-khaki-300/80"></i>
      </div>
      <p class="text-4xl sm:text-5xl font-black text-white leading-none tracking-tight">₹48,600</p>
      <p class="text-[11px] text-slate-400 mt-1.5 mb-4">Sum of six independently priced modules</p>
      <div class="space-y-2">
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-microchip text-indigo-300 mr-2"></i>Motherboard &amp; GPU</span><span class="text-xs font-black text-white">₹17,010</span></div>
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-display text-sky-300 mr-2"></i>OLED 3.5K Display</span><span class="text-xs font-black text-white">₹10,692</span></div>
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-hard-drive text-amber-300 mr-2"></i>1TB NVMe SSD</span><span class="text-xs font-black text-white">₹6,318</span></div>
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-battery-three-quarters text-emerald-300 mr-2"></i>86Wh Battery Pack</span><span class="text-xs font-black text-white">₹5,832</span></div>
      </div>
      <p class="text-[10px] text-slate-400 mt-3.5 text-center">+ 32GB RAM &amp; CNC chassis · 14-day price lock</p>
    `;
    if (btnPhone) btnPhone.className = 'px-3 py-1 rounded-lg text-xs font-bold text-slate-300 hover:text-white transition-all';
    if (btnLaptop) btnLaptop.className = 'px-3 py-1 rounded-lg text-xs font-bold bg-[#A85B28] text-white shadow-sm transition-all';
  } else {
    card.innerHTML = `
      <div class="flex items-start justify-between gap-4 mb-3">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[.16em] text-khaki-300">Sample Phone Payout</p>
          <p class="text-xs text-slate-300 mt-0.5">iPhone 14 Pro · 256GB · Good</p>
        </div>
        <i class="fa-solid fa-mobile-screen-button text-2xl text-khaki-300/80"></i>
      </div>
      <p class="text-4xl sm:text-5xl font-black text-white leading-none tracking-tight">₹29,100</p>
      <p class="text-[11px] text-slate-400 mt-1.5 mb-4">Sum of five independently priced parts</p>
      <div class="space-y-2">
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-display text-sky-300 mr-2"></i>Display</span><span class="text-xs font-black text-white">₹9,400</span></div>
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-camera text-amber-300 mr-2"></i>Camera</span><span class="text-xs font-black text-white">₹5,200</span></div>
        <div class="val-row"><span class="text-xs font-bold text-slate-200"><i class="fa-solid fa-battery-three-quarters text-rose-300 mr-2"></i>Battery</span><span class="text-xs font-black text-white">₹2,300</span></div>
      </div>
      <p class="text-[10px] text-slate-400 mt-3.5 text-center">+ logic board &amp; housing · 14-day price lock</p>
    `;
    if (btnPhone) btnPhone.className = 'px-3 py-1 rounded-lg text-xs font-bold bg-[#A85B28] text-white shadow-sm transition-all';
    if (btnLaptop) btnLaptop.className = 'px-3 py-1 rounded-lg text-xs font-bold text-slate-300 hover:text-white transition-all';
  }
}

function setHomeBreakdownDevice(type) {
  const phoneView = document.getElementById('breakdown-phone-view');
  const laptopView = document.getElementById('breakdown-laptop-view');
  const btnPhone = document.getElementById('btn-breakdown-phone');
  const btnLaptop = document.getElementById('btn-breakdown-laptop');
  if (type === 'laptop') {
...
function renderDatabaseUI() {
  ...
  if (quotes.length === 0) {
    qList.innerHTML = '<div class="p-6 text-center bg-slate-50 rounded-2xl border border-slate-200 text-slate-400 text-xs font-semibold">No active quotations yet. Value a smartphone or laptop to lock in your price.</div>';
    document.getElementById('home-saved-quote-title').textContent = 'No Active Quote';
    document.getElementById('home-saved-quote-price').textContent = 'Value a phone or laptop to lock price';
  } else {
      ...
      <div id="order-card-${o.id}" class="p-4 rounded-2xl border border-slate-200 bg-slate-50/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500">Order #${o.id} · Certified by PaciFIQ</p>
...
function computeQuote() {
  ...
  // 1. Base Market Value from Tiered Dictionary
  const baseMarketValue = MODEL_MSRP[model] || (brand === 'apple' ? 85000 : (brand === 'realme' ? 12000 : 35000));

  // 2. Age Depreciation Factor based on tiered retention and decay
  const { decay, retention } = getTierDecayParams(baseMarketValue);
  const ageDepreciationFactor = Math.max(0.10, Math.round(Math.pow(1 - decay, ageMonths) * retention * 10000) / 10000);

  // 3. Condition Multiplier from component grade score
  const tags = {}, deducts = [];
  ...
  GRADE_GROUPS.forEach(g => {
    ...
    conditionMultiplier += g.weight * opt.mult;
  });

  const pct  = Math.round(conditionMultiplier * 100);
  const band = GRADE_BANDS.find(b => pct >= b.min) || GRADE_BANDS[GRADE_BANDS.length - 1];

  // FORMULA: Final Payout = Base Market Value × Age Depreciation Factor × Condition Multiplier
  const finalPayout = Math.round(baseMarketValue * ageDepreciationFactor * conditionMultiplier);
  const mintPayout  = Math.round(baseMarketValue * ageDepreciationFactor * 1.00);

  // 4. Component-Level Decomposition (Display, Logic, Camera, Battery, Chassis sum up to 100% of Final Payout)
  const vals = {}, mintVals = {};
  GRADE_GROUPS.forEach(g => {
    const weight = COMPONENT_WEIGHTS[g.part] || 0.20;
    const mintV = Math.round(mintPayout * weight);
    const v = Math.round(finalPayout * weight);
    mintVals[g.part] = mintV;
    vals[g.part]     = v;
    if (mintV > v) deducts.push({ title: g.title, label: gradeOption(g).label, amount: mintV - v });
  });

  // Ensure exact penny mathematical equality so sum(vals) === finalPayout
  const currentSum = Object.values(vals).reduce((a, b) => a + b, 0);
  if (vals.logic) vals.logic += (finalPayout - currentSum);

  const mintSum = Object.values(mintVals).reduce((a, b) => a + b, 0);
  if (mintVals.logic) mintVals.logic += (mintPayout - mintSum);

  return {
    brand, model, ageMonths, buyYear, vals, mintVals, tags, deducts, band, pct,
    baseMarketValue, ageDepreciationFactor, conditionMultiplier,
    gross:     finalPayout,
    net:       finalPayout,
    mintGross: mintPayout,
    mintNet:   mintPayout,
...
const Teardown3D = (function () {
  ...
  function frame(now) {
    ...
    /* The slow idle drift is worth a frame only while somebody is actually
       at the machine. Tweens and drags always finish; everything else stops
       with the activity clock, and `raf` is cleared so kick() can restart
       the chain rather than believing one is already pending. */
    if (tweening || drag.on || (visible && !reduced && Activity.active()))
  ...
  function init() {
    ...
    /* antialias and a 2x pixel ratio are both pure fragment cost, and this is
       a lightbox render of flat plates where neither earns its keep on a weak
       GPU. 1.5x is the same cap the cursor grid and the ribbon already use. */
    const lowfi = document.documentElement.classList.contains('lowfi');
    try {
      renderer = new THREE.WebGLRenderer({ canvas, antialias: !lowfi, alpha: true,
                                           powerPreference: 'low-power' });
    } catch (e) { return false; }
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, lowfi ? 1 : 1.5));
...
function renderTrendChartRealistic(brand, mult, ageMonths) {
  ...
  S.chartInst = new Chart(ctx, {
    ...
    options: {
      ...
      animation: (reduceMotion || document.documentElement.classList.contains('lowfi')) ? false : {
        duration: 900,
        easing: 'easeOutQuart',
...
/* ── 6e. Device type toggle ─────────────────────────────────────────────── */

function setDeviceType(type) {
  const phoneSec = document.getElementById('phone-sell-section');
  const laptopSec = document.getElementById('laptop-sell-section');
  const btnPhone = document.getElementById('dtype-phone');
  const btnLaptop = document.getElementById('dtype-laptop');
  if (type === 'phone') {
    phoneSec.classList.remove('hidden');
    laptopSec.classList.add('hidden');
    btnPhone.className = 'px-6 py-2.5 rounded-xl font-bold text-sm bg-[#284139] text-white shadow-sm transition-all';
    btnLaptop.className = 'px-6 py-2.5 rounded-xl font-bold text-sm text-slate-600 hover:text-slate-900 transition-all';
    resetSellAndDisassembleToDefault();
  } else {
    phoneSec.classList.add('hidden');
    laptopSec.classList.remove('hidden');
    btnLaptop.className = 'px-6 py-2.5 rounded-xl font-bold text-sm bg-[#284139] text-white shadow-sm transition-all';
    btnPhone.className = 'px-6 py-2.5 rounded-xl font-bold text-sm text-slate-600 hover:text-slate-900 transition-all';
    resetLaptopToDefault();
    setTimeout(function() { initLaptopTeardown3D(); }, 120);
  }
}

/* ─────────────────────────────────────────────────────────────────────────
   LAPTOP ENGINE — Catalog, BOM, 3D Teardown & Valuation
   ───────────────────────────────────────────────────────────────────────── */

const LAPTOP_BRANDS = {
  apple_mac: [
    'MacBook Air 15" M3 (2024)', 'MacBook Air 13" M3 (2024)', 'MacBook Pro 16" M3 Pro (2024)',
    'MacBook Pro 14" M3 (2024)', 'MacBook Air 13" M2 (2022)', 'MacBook Pro 16" M2 Pro (2023)',
    'MacBook Pro 14" M2 Pro (2023)', 'MacBook Air M1 (2020)', 'MacBook Pro 13" M1 (2020)',
    'MacBook Pro 16" Intel i9 (2019)', 'MacBook Air 13" Intel i5 (2020)', 'MacBook 12" (2017 Vintage)'
  ],
  dell: [
    'Dell XPS 15 (2024, RTX 4070)', 'Dell XPS 13 Plus (2024)', 'Dell XPS 15 (2022, i9)',
    'Dell Alienware m16 R2 (2024)', 'Dell Latitude 7440 (2023)', 'Dell Inspiron 16 Plus (2023)',
    'Dell G15 Gaming (RTX 3060)', 'Dell Precision 5680 (2023)', 'Dell XPS 17 (2021, i7)',
    'Dell Inspiron 15 3520 (2022)', 'Dell XPS 13 (2018 Vintage)'
  ],
  hp: [
    'HP Spectre x360 14" (2024)', 'HP Envy 16 (2024)', 'HP EliteBook 840 G10 (2023)',
    'HP Omen 16 (RTX 4070)', 'HP ProBook 450 G10 (2023)', 'HP Pavilion 15 (i7-1355U)',
    'HP Dragonfly Pro (2023)', 'HP ZBook Studio G10 (2023)', 'HP Spectre x360 13" (2021)',
    'HP Victus 15 (RTX 3050)', 'HP EliteBook 840 G5 (2018 Legacy)'
  ],
  lenovo: [
    'Lenovo ThinkPad X1 Carbon Gen 12 (2024)', 'Lenovo ThinkPad T14s Gen 5 (2024)',
    'Lenovo Yoga 9i Gen 9 (2024)', 'Lenovo LOQ 15 (RTX 4060)', 'Lenovo IdeaPad Slim 5i (2024)',
    'Lenovo Legion Pro 7i Gen 9 (RTX 4080)', 'Lenovo ThinkPad X1 Extreme Gen 5 (2022)',
    'Lenovo Yoga Slim 7 Pro (2022)', 'Lenovo IdeaPad Gaming 3 (RTX 3050)',
    'Lenovo ThinkPad E14 Gen 5 (2023)', 'Lenovo ThinkPad X220 (2011 Vintage)'
  ],
  asus: [
    'ASUS ROG Zephyrus G16 (2024, RTX 4090)', 'ASUS ProArt Studiobook 16 (RTX 4070)',
    'ASUS Vivobook Pro 15 OLED (2024)', 'ASUS ROG Strix G18 (RTX 4060)',
    'ASUS ZenBook 14 OLED (2024)', 'ASUS TUF Gaming F15 (RTX 3050)',
    'ASUS ExpertBook B9 (2023)', 'ASUS Chromebook Flip C436', 'ASUS VivoBook 15 (i5-1135G7)'
  ],
  acer: [
    'Acer Swift X 14 (RTX 4070, 2024)', 'Acer Predator Helios 16 (RTX 4080)',
    'Acer ConceptD 7 Pro (RTX 4090)', 'Acer Nitro 5 (RTX 3060)', 'Acer Aspire 5 (i7-1255U)',
    'Acer TravelMate P6 (2023)', 'Acer Swift Go 14 (2023)', 'Acer Chromebook 516 GE'
  ],
  microsoft: [
    'Surface Laptop 7 15" (2024)', 'Surface Laptop 7 13.8" (2024)', 'Surface Pro 11" (2024)',
    'Surface Laptop Studio 2 (2023)', 'Surface Laptop 5 13.5" (2022)', 'Surface Pro 9 (2022)',
    'Surface Book 3 (2020)', 'Surface Laptop 3 (2019 Vintage)'
  ],
  msi: [
    'MSI Titan GT77 HX (RTX 4090)', 'MSI Raider GE78 HX (2024)', 'MSI Stealth 16 Studio',
    'MSI Prestige 14 AI+ (2024)', 'MSI Creator Z16 (RTX 3060)', 'MSI Vector GP76 (RTX 3070)'
  ]
};

const LAPTOP_MSRP = {
  'MacBook Air 15" M3 (2024)': 134900,
  'MacBook Air 13" M3 (2024)': 114900,
  'MacBook Pro 16" M3 Pro (2024)': 249900,
  'MacBook Pro 14" M3 (2024)': 168900,
  'MacBook Air 13" M2 (2022)': 99900,
  'MacBook Pro 16" M2 Pro (2023)': 249900,
  'MacBook Pro 14" M2 Pro (2023)': 199900,
  'MacBook Air M1 (2020)': 84900,
  'MacBook Pro 13" M1 (2020)': 119900,
  'MacBook Pro 16" Intel i9 (2019)': 80000,
  'MacBook Air 13" Intel i5 (2020)': 55000,
  'MacBook 12" (2017 Vintage)': 25000,
  'Dell XPS 15 (2024, RTX 4070)': 189990,
  'Dell XPS 13 Plus (2024)': 149990,
  'Dell XPS 15 (2022, i9)': 139990,
  'Dell Alienware m16 R2 (2024)': 229990,
  'Dell Latitude 7440 (2023)': 129990,
  'Dell Inspiron 16 Plus (2023)': 89990,
  'Dell G15 Gaming (RTX 3060)': 79990,
  'Dell Precision 5680 (2023)': 189990,
  'Dell XPS 17 (2021, i7)': 120000,
  'Dell Inspiron 15 3520 (2022)': 54990,
  'Dell XPS 13 (2018 Vintage)': 40000,
  'HP Spectre x360 14" (2024)': 189990,
  'HP Envy 16 (2024)': 129990,
  'HP EliteBook 840 G10 (2023)': 139990,
  'HP Omen 16 (RTX 4070)': 159990,
  'HP ProBook 450 G10 (2023)': 79990,
  'HP Pavilion 15 (i7-1355U)': 64990,
  'HP Dragonfly Pro (2023)': 189990,
  'HP ZBook Studio G10 (2023)': 249990,
  'HP Spectre x360 13" (2021)': 90000,
  'HP Victus 15 (RTX 3050)': 64990,
  'HP EliteBook 840 G5 (2018 Legacy)': 35000,
  'Lenovo ThinkPad X1 Carbon Gen 12 (2024)': 189990,
  'Lenovo ThinkPad T14s Gen 5 (2024)': 129990,
  'Lenovo Yoga 9i Gen 9 (2024)': 159990,
  'Lenovo LOQ 15 (RTX 4060)': 89990,
  'Lenovo IdeaPad Slim 5i (2024)': 59990,
  'Lenovo Legion Pro 7i Gen 9 (RTX 4080)': 279990,
  'Lenovo ThinkPad X1 Extreme Gen 5 (2022)': 180000,
  'Lenovo Yoga Slim 7 Pro (2022)': 99990,
  'Lenovo IdeaPad Gaming 3 (RTX 3050)': 64990,
  'Lenovo ThinkPad E14 Gen 5 (2023)': 84990,
  'Lenovo ThinkPad X220 (2011 Vintage)': 12000,
  'ASUS ROG Zephyrus G16 (2024, RTX 4090)': 279990,
  'ASUS ProArt Studiobook 16 (RTX 4070)': 229990,
  'ASUS Vivobook Pro 15 OLED (2024)': 89990,
  'ASUS ROG Strix G18 (RTX 4060)': 139990,
  'ASUS ZenBook 14 OLED (2024)': 79990,
  'ASUS TUF Gaming F15 (RTX 3050)': 64990,
  'ASUS ExpertBook B9 (2023)': 129990,
  'ASUS Chromebook Flip C436': 45000,
  'ASUS VivoBook 15 (i5-1135G7)': 49990,
  'Acer Swift X 14 (RTX 4070, 2024)': 129990,
  'Acer Predator Helios 16 (RTX 4080)': 199990,
  'Acer ConceptD 7 Pro (RTX 4090)': 329990,
  'Acer Nitro 5 (RTX 3060)': 79990,
  'Acer Aspire 5 (i7-1255U)': 54990,
  'Acer TravelMate P6 (2023)': 89990,
  'Acer Swift Go 14 (2023)': 74990,
  'Acer Chromebook 516 GE': 52990,
  'Surface Laptop 7 15" (2024)': 189990,
  'Surface Laptop 7 13.8" (2024)': 159990,
  'Surface Pro 11" (2024)': 179990,
  'Surface Laptop Studio 2 (2023)': 219990,
  'Surface Laptop 5 13.5" (2022)': 109990,
  'Surface Pro 9 (2022)': 139990,
  'Surface Book 3 (2020)': 120000,
  'Surface Laptop 3 (2019 Vintage)': 55000,
  'MSI Titan GT77 HX (RTX 4090)': 399990,
  'MSI Raider GE78 HX (2024)': 249990,
  'MSI Stealth 16 Studio': 199990,
  'MSI Prestige 14 AI+ (2024)': 149990,
  'MSI Creator Z16 (RTX 3060)': 129990,
  'MSI Vector GP76 (RTX 3070)': 139990
};

const LAPTOP_COMPONENT_WEIGHTS = {
  motherboard: 0.35,
  display:     0.22,
  battery:     0.12,
  storage:     0.13,
...
const LAPTOP_GRADE_GROUPS = [
  {
    key: 'display', part: 'display', weight: 0.25,
    icon: 'fa-display', title: 'Display / Screen', sub: 'How does the panel look?',
    options: [
      { v: 'flawless', label: 'Flawless',         desc: 'No dead pixels, no scratches',      mult: 1.00 },
      { v: 'light',    label: 'Light scratches',   desc: 'Faint marks, not visible in use',   mult: 0.86 },
      { v: 'deep',     label: 'Deep scratches',    desc: 'Visible marks on glass',            mult: 0.65 },
      { v: 'cracked',  label: 'Cracked / broken',  desc: 'Panel cracked or delaminated',      mult: 0.32 },
      { v: 'dead',     label: 'Dead / no display', desc: 'Backlight failed or panel dead',    mult: 0.12 }
    ]
  },
  {
    key: 'chassis', part: 'chassis', weight: 0.15,
    icon: 'fa-layer-group', title: 'Body / Chassis', sub: 'What is the body condition?',
    options: [
      { v: 'mint',   label: 'Mint',          desc: 'No dents or scratches',             mult: 1.00 },
      { v: 'scuffs', label: 'Minor scuffs',   desc: 'Small marks on edges/lid',          mult: 0.88 },
      { v: 'dents',  label: 'Dents or bends', desc: 'Noticeable damage to frame',        mult: 0.62 },
      { v: 'severe', label: 'Severe damage',  desc: 'Cracked casing, broken hinges',     mult: 0.30 }
    ]
  },
  {
    key: 'battery', part: 'battery', weight: 0.20,
    icon: 'fa-battery-three-quarters', title: 'Battery', sub: 'What is the battery health?',
    options: [
      { v: 'high',    label: '85% or above',       desc: 'Near-new capacity',                 mult: 1.00 },
      { v: 'mid',     label: '70 to 84%',          desc: 'Normal for age',                    mult: 0.75 },
      { v: 'low',     label: 'Below 70%',          desc: 'Reduced runtime',                   mult: 0.45 },
      { v: 'service', label: 'Swelling / failing', desc: 'Needs immediate replacement',       mult: 0.15 }
    ]
  },
  {
    key: 'keyboard', part: 'chassis', weight: 0.10,
    icon: 'fa-keyboard', title: 'Keyboard & Trackpad', sub: 'Do all keys and trackpad work?',
    options: [
      { v: 'perfect', label: 'All perfect',         desc: 'Every key and gesture works',       mult: 1.00 },
      { v: 'worn',    label: 'Worn keycaps',         desc: 'Legends faded but functional',      mult: 0.88 },
      { v: 'sticky',  label: 'Sticky / dead keys',  desc: 'One or more keys malfunction',      mult: 0.55 },
      { v: 'broken',  label: 'Trackpad broken',     desc: 'Trackpad non-functional',           mult: 0.40 }
    ]
  },
  {
    key: 'logic', part: 'motherboard', weight: 0.30,
    icon: 'fa-microchip', title: 'CPU / Motherboard', sub: 'How does the system perform?',
    options: [
      { v: 'ok',       label: 'Boots, runs fine',   desc: 'All ports and sensors respond',    mult: 1.00 },
      { v: 'minor',    label: 'Minor issues',        desc: 'Minor thermal throttling or lag',  mult: 0.80 },
      { v: 'unstable', label: 'Unstable / crashes',  desc: 'BSODs or system hangs under load',mult: 0.48 },
      { v: 'dead',     label: 'Will not boot',       desc: 'No POST, no display, no power',   mult: 0.15 }
    ]
  }
];

const LAPTOP_GRADE_BANDS = [
  { min: 95, label: 'Mint',      cond: 'flawless', ring: '#F8D794' },
  { min: 82, label: 'Excellent', cond: 'flawless', ring: '#CFB98C' },
  { min: 68, label: 'Good',      cond: 'good',     ring: '#809076' },
  { min: 48, label: 'Fair',      cond: 'cracked',  ring: '#E5A876' },
  { min: 0,  label: 'Salvage',   cond: 'dead',     ring: '#D2874C' }
];

const LP_EXPLODE_PARTS = [
  ['lp-ex-display',     'display',     'Display Panel',   'fa-display'],
  ['lp-ex-motherboard', 'motherboard', 'Motherboard',     'fa-microchip'],
  ['lp-ex-battery',     'battery',     'Battery Pack',    'fa-battery-three-quarters'],
  ['lp-ex-storage',     'storage',     'SSD Storage',     'fa-hard-drive'],
  ['lp-ex-ram',         'ram',         'RAM Module',      'fa-memory'],
  ['lp-ex-chassis',     'chassis',     'Chassis & Frame', 'fa-crop-simple']
];

const defaultLaptopGrades = function() {
  return LAPTOP_GRADE_GROUPS.reduce(function(acc, g) { acc[g.key] = g.options[0].v; return acc; }, {});
};

const lpGradeOption = function(g) {
  return g.options.find(function(o) { return o.v === S.lpGrades[g.key]; }) || g.options[0];
};

S.lpGrades = {};
S.lpTearingDown = false;
S.lpCurrentModel = '';
S.lpChartInst = null;

function updateLaptopModels() {
  var brand = document.getElementById('lp-brand').value;
  var sel = document.getElementById('lp-model');
  sel.innerHTML = '<option value="" disabled selected>(Choose Model)</option>';
  if (brand && LAPTOP_BRANDS[brand]) {
    LAPTOP_BRANDS[brand].forEach(function(m) {
...
function renderLaptopGradeCards() {
  var host = document.getElementById('lp-grade-cards');
  if (!host) return;
  if (!S.lpGrades || Object.keys(S.lpGrades).length === 0) S.lpGrades = defaultLaptopGrades();
  host.innerHTML = LAPTOP_GRADE_GROUPS.map(function(g, gi) { return `
    <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
      <div class="flex items-start gap-3 mb-4">
        <span class="w-9 h-9 rounded-xl bg-emerald-50 border border-khaki-200 flex items-center justify-center text-[#284139] shrink-0">
          <i class="fa-solid ${g.icon} text-sm"></i>
        ...
        <span class="ml-auto text-[10px] font-bold uppercase tracking-wider text-slate-400 whitespace-nowrap">${gi + 1} / ${LAPTOP_GRADE_GROUPS.length}</span>
      </div>
      <div class="grid sm:grid-cols-2 lg:grid-cols-1 gap-2.5">
        ${g.options.map(function(o) { return `
        <button type="button" onclick="setLaptopGrade('${g.key}','${o.v}')"
                data-lpgrade="${g.key}" data-val="${o.v}"
                aria-pressed="${S.lpGrades[g.key] === o.v}"
                class="grade-tile${S.lpGrades[g.key] === o.v ? ' on' : ''}">
          <span class="grade-tile-txt">
            <span class="grade-tile-label">${o.label}</span>
            <span class="grade-tile-desc">${o.desc}</span>
          </span>
...
function setLaptopGrade(key, value) {
  S.lpGrades[key] = value;
  document.querySelectorAll('[data-lpgrade="' + key + '"]').forEach(function(el) {
    var on = el.dataset.val === value;
    el.classList.toggle('on', on);
    el.setAttribute('aria-pressed', String(on));
  });
  refreshLaptopEstimate();
}

function resetLaptopGrades() {
  S.lpGrades = defaultLaptopGrades();
  renderLaptopGradeCards();
  refreshLaptopEstimate();
  toast('Laptop grades reset to mint.');
}

function computeLaptopQuote() {
  var val = function(id, fb) { var el = document.getElementById(id); return (el && el.value) ? el.value : fb; };
  var brand    = val('lp-brand', 'dell');
  var model    = val('lp-model', 'Dell XPS 15 (2024, RTX 4070)');
  var buyYear  = parseInt(val('lp-buy-year', '2023'), 10);
  var buyMonth = parseInt(val('lp-buy-month', '8'), 10);
  var nowYear  = 2026, nowMonth = 8;
  var ageMonths = Math.max(1, (nowYear - buyYear) * 12 + (nowMonth - buyMonth));

  var msrp = LAPTOP_MSRP[model] || 75000;
  var monthlyDecay = msrp >= 150000 ? 0.0125 : (msrp >= 70000 ? 0.0140 : 0.0155);
  var ageDepreciationFactor = Math.max(0.08, Math.pow(1 - monthlyDecay, ageMonths));

  if (!S.lpGrades || Object.keys(S.lpGrades).length === 0) S.lpGrades = defaultLaptopGrades();

  var conditionMultiplier = 0;
  var tags = {}, deducts = [];
  LAPTOP_GRADE_GROUPS.forEach(function(g) {
    var opt = lpGradeOption(g);
    tags[g.key] = opt.label;
    conditionMultiplier += g.weight * opt.mult;
  });

  var pct  = Math.round(conditionMultiplier * 100);
  var band = LAPTOP_GRADE_BANDS.find(function(b) { return pct >= b.min; }) || LAPTOP_GRADE_BANDS[LAPTOP_GRADE_BANDS.length - 1];

  var finalPayout = Math.round(msrp * ageDepreciationFactor * conditionMultiplier);
  var mintPayout  = Math.round(msrp * ageDepreciationFactor);

  var vals = {}, mintVals = {};
  Object.keys(LAPTOP_COMPONENT_WEIGHTS).forEach(function(part) {
    mintVals[part] = Math.round(mintPayout * LAPTOP_COMPONENT_WEIGHTS[part]);
    vals[part]     = Math.round(finalPayout * LAPTOP_COMPONENT_WEIGHTS[part]);
  });
  var curSum = Object.values(vals).reduce(function(a, b) { return a + b; }, 0);
  if (vals.motherboard !== undefined) vals.motherboard += (finalPayout - curSum);
  var mintSum = Object.values(mintVals).reduce(function(a, b) { return a + b; }, 0);
  if (mintVals.motherboard !== undefined) mintVals.motherboard += (mintPayout - mintSum);

  var partMap = { display: 'display', chassis: 'chassis', battery: 'battery', keyboard: 'chassis', logic: 'motherboard' };
  LAPTOP_GRADE_GROUPS.forEach(function(g) {
    var opt = lpGradeOption(g);
    var part = partMap[g.key] || g.key;
    var mintV = mintVals[part] || 0;
    var v = vals[part] || 0;
    if (mintV > v) deducts.push({ title: g.title, label: opt.label, amount: mintV - v });
  });

  return { brand, model, ageMonths, buyYear, vals, mintVals, tags, deducts, band, pct,
           msrp, ageDepreciationFactor, conditionMultiplier,
           gross: finalPayout, net: finalPayout, mintGross: mintPayout, mintNet: mintPayout };
}

function setLpPartValue(key, text) {
  var el = document.getElementById('lp-ex-' + key);
  if (el) el.textContent = text;
  if (LaptopTeardown3D.ready) LaptopTeardown3D.setValue(key, text);
}

function refreshLaptopEstimate() {
  var q = computeLaptopQuote();
  var set = function(id, txt) { var el = document.getElementById(id); if (el) el.textContent = txt; };
  var rupees = function(n) { return '\u20b9' + n.toLocaleString('en-IN'); };

  set('lp-grade-value', rupees(q.net));
  set('lp-grade-value-sub', q.model + ' \u00b7 after 18% platform fee');
  set('lp-grade-age-chip', q.ageMonths + ' months old');
  set('lp-grade-score', q.pct);
  set('lp-grade-band', q.band.label);
  set('lp-grade-mint', rupees(q.mintNet));

  var ring = document.getElementById('lp-grade-ring');
  if (ring) {
    var c = 2 * Math.PI * 26;
    ring.style.strokeDasharray = (q.pct / 100 * c).toFixed(1) + ' ' + c.toFixed(1);
    ring.style.stroke = q.band.ring;
  }
  var bandEl = document.getElementById('lp-grade-band');
  if (bandEl) bandEl.style.color = q.band.ring;

  var rows = document.getElementById('lp-bom-rows');
  if (rows) {
    rows.innerHTML = LP_EXPLODE_PARTS.map(function(p) {
      var key = p[1], title = p[2], icon = p[3];
      var v = q.vals[key] || 0;
      var pct = q.gross ? Math.round((v / q.gross) * 100) : 0;
      return '<div class="p-2.5 rounded-xl bg-white/5 border border-white/10">' +
        '<div class="flex justify-between text-xs mb-1.5">' +
        '<span class="font-bold text-white flex items-center gap-1.5"><i class="fa-solid ' + icon + ' text-khaki-300"></i>' + title + '</span>' +
        '<span class="font-black text-khaki-300">' + rupees(v) + '</span></div>' +
        '<div class="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">' +
        '<div class="h-full bg-khaki-300 rounded-full transition-all duration-500" style="width:' + pct + '%"></div></div>' +
        '<div class="flex justify-between text-[10px] text-slate-400 mt-1"><span>' + pct + '% of BOM</span></div></div>';
    }).join('');
  }

  var list = document.getElementById('lp-grade-deducts');
  if (list) {
    var seen = {};
    var unique = q.deducts.filter(function(d) { if (seen[d.title]) return false; seen[d.title] = 1; return true; });
    list.innerHTML = unique.length
      ? unique.map(function(d) { return '<div class="flex items-center justify-between gap-3 text-xs py-1"><span class="text-slate-300 truncate"><span class="font-bold text-white">' + d.title + '</span> \u00b7 ' + d.label + '</span><span class="font-black text-terra-300 whitespace-nowrap">\u2212' + rupees(d.amount) + '</span></div>'; }).join('')
      : '<p class="text-xs font-bold text-khaki-300 py-1">No deductions \u2014 graded as mint.</p>';
  }

  var condField = document.getElementById('lp-cond');
  if (condField) condField.value = q.band.cond;

  if (lpIsTornDown()) LP_EXPLODE_PARTS.forEach(function(p) { setLpPartValue(p[1], rupees(q.vals[p[1]] || 0)); });
  return q;
}

var lpIsTornDown = function() {
  if (LaptopTeardown3D.ready) return LaptopTeardown3D.exploded;
  var svg = document.getElementById('lp-explode');
  return !!svg && svg.classList.contains('exploded');
};

function runLaptopDisassembly() {
  if (S.lpTearingDown) return;
  var q = refreshLaptopEstimate();
  S.lpCurrentModel = q.model;
  S.lpTearingDown = true;

  var stage = document.getElementById('lp-disassembly-stage');
  if (stage) stage.scrollIntoView({ behavior: prefersReducedMotion() ? 'auto' : 'smooth', block: 'center' });

  var badge = document.getElementById('lp-dis-badge');
  if (badge) { badge.textContent = 'Separating hardware\u2026'; badge.className = 'text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-800'; }

  var quotes = window.DB.get('quotes', []);
  var existing = quotes.findIndex(function(x) { return x.title === q.model; });
  if (existing >= 0) { quotes[existing].price = q.net; quotes[existing].condition = q.band.cond; }
  else { quotes.unshift({ id: 'q-' + Date.now(), title: q.model, condition: q.band.cond, price: q.net, status: 'Quote Locked', hub: 'Bhatpara Central' }); }
  window.DB.set('quotes', quotes);

  playLaptopExplosion(q, function() { S.lpTearingDown = false; applyLaptopTeardownResults(q); });
}

function playLaptopExplosion(q, onSettled) {
  var use3D = LaptopTeardown3D.ready;
  var svg   = document.getElementById('lp-explode');
  var hint  = document.getElementById('lp-explode-hint');
  var price = function(key) { return '\u20b9' + (q.vals[key] || 0).toLocaleString('en-IN'); };
  if (!use3D && !svg) { onSettled(); return; }

  if (prefersReducedMotion()) {
    LP_EXPLODE_PARTS.forEach(function(p) { setLpPartValue(p[1], price(p[1])); });
    if (use3D) LaptopTeardown3D.explode(); else svg.classList.add('exploded');
    if (hint) hint.textContent = 'Every part separated and valued at today\'s component spot rates.';
    onSettled(); return;
  }

  if (!use3D) svg.classList.remove('exploded');
  if (hint) hint.textContent = 'Releasing screws and lifting the lid assembly\u2026';

  requestAnimationFrame(function() {
    if (use3D) LaptopTeardown3D.explode(); else svg.classList.add('exploded');
    LP_EXPLODE_PARTS.forEach(function(p, i) {
      setTimeout(function() {
        var key = p[1], t0 = performance.now(), target = q.vals[key] || 0;
        var step = function(t) {
          var prog = Math.min((t - t0) / 620, 1);
          var eased = 1 - Math.pow(1 - prog, 3);
          setLpPartValue(key, '\u20b9' + Math.round(target * eased).toLocaleString('en-IN'));
  ...
  setTimeout(function() {
    if (hint) hint.textContent = 'Every part separated and valued at today\'s component spot rates.';
    onSettled();
  }, (use3D ? LaptopTeardown3D.durationMs : 1700) + 120);
}

function applyLaptopTeardownResults(q) {
  var badge = document.getElementById('lp-dis-badge');
  if (badge) { badge.textContent = '\u2713 Hardware Disassembled'; badge.className = 'text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800'; }
  var ageLabel = document.getElementById('lp-bom-age-label');
  if (ageLabel) ageLabel.textContent = 'Quote locked \u00b7 Age ' + q.ageMonths + ' months (' + q.buyYear + ') \u00b7 Grade ' + q.band.label + ' ' + q.pct + '/100 \u00b7 Fee 18%';
  renderLaptopTrendChart(q.brand, q.conditionMultiplier, q.ageMonths);
  toast('Teardown calculated for ' + q.model + ': \u20b9' + q.net.toLocaleString('en-IN'));
}

function renderLaptopTrendChart(brand, condMult, ageMonths) {
  var canvas = document.getElementById('lp-trend-chart');
  if (!canvas) return;
  var labels = Array.from({ length: 12 }, function(_, i) { return 'W' + (i + 1); });
  var base = LAPTOP_MSRP['Dell XPS 15 (2024, RTX 4070)'] || 90000;
  var genSeries = function(weight, noise, trend) {
    return labels.map(function(_, i) {
      return Math.max(800, Math.round(base * weight * condMult *
        (1 - (ageMonths + i * 0.8) * 0.012) + (Math.random() - 0.5) * noise * base + trend * i));
    });
  };
  var datasets = [
    { label: 'Display Panel', data: genSeries(0.22, 0.012, -18), borderColor: '#F8D794', backgroundColor: 'rgba(248,215,148,.09)', tension: 0.4 },
    { label: 'Motherboard',   data: genSeries(0.35, 0.016, -28), borderColor: '#809076', backgroundColor: 'rgba(128,144,118,.09)', tension: 0.4 },
    { label: 'Battery Pack',  data: genSeries(0.12, 0.008, -8),  borderColor: '#B86830', backgroundColor: 'rgba(184,104,48,.09)',  tension: 0.4 },
    { label: 'SSD Storage',   data: genSeries(0.13, 0.009, -10), borderColor: '#CFB98C', backgroundColor: 'rgba(207,185,140,.09)', tension: 0.4 }
  ];
  if (S.lpChartInst) S.lpChartInst.destroy();
  S.lpChartInst = new Chart(canvas, {
    type: 'line', data: { labels, datasets },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false } },
      scales: {
        x: { grid: { color: 'rgba(40,65,57,.1)' }, ticks: { font: { size: 10 } } },
        y: { grid: { color: 'rgba(40,65,57,.1)' }, ticks: { font: { size: 10 }, callback: function(v) { return '\u20b9' + (v / 1000).toFixed(0) + 'k'; } } }
      }
    }
  });
}

function filterLaptopChart(idx) {
  document.querySelectorAll('#laptop-sell-section .brand-pill').forEach(function(b) { b.classList.remove('active'); });
  if (!S.lpChartInst) return;
  if (idx === 'all') S.lpChartInst.data.datasets.forEach(function(d) { d.hidden = false; });
  else S.lpChartInst.data.datasets.forEach(function(d, i) { d.hidden = i !== idx; });
  S.lpChartInst.update();
}

function resetLaptopToDefault() {
  var svg = document.getElementById('lp-explode');
  if (svg) svg.classList.remove('exploded');
  if (LaptopTeardown3D.ready) LaptopTeardown3D.assemble();
  var hint = document.getElementById('lp-explode-hint');
  if (hint) hint.textContent = 'Grade the components, then run the teardown to separate and price every part.';
  var badge = document.getElementById('lp-dis-badge');
  if (badge) { badge.textContent = 'Ready to Analyze'; badge.className = 'text-xs font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-600'; }
  LP_EXPLODE_PARTS.forEach(function(p) { setLpPartValue(p[1], '\u20b90'); });
  var lbl = document.getElementById('lp-bom-age-label');
  if (lbl) lbl.textContent = 'Click "Check & Disassemble Laptop" to calculate';
  S.lpTearingDown = false;
  S.lpGrades = defaultLaptopGrades();
  renderLaptopGradeCards();
  refreshLaptopEstimate();
  renderLaptopTrendChart('dell', 0.60, 36);
}

/* ─────────────────────────────────────────────────────────────────────────
   LAPTOP TEARDOWN 3D — WebGL exploded view (Three.js)
   ─────────────────────────────────────────────────────────────────────────
   Procedural laptop: lid (display), base chassis, motherboard,
   battery pack, SSD strip, RAM sticks. Parts separate vertically on demand.
   ───────────────────────────────────────────────────────────────────────── */
var LaptopTeardown3D = (function () {
  var EXPLODE_MS = 1600;
  ...
  var M = {
    lid:       { color: 0x1E2B25, metalness: 0.88, roughness: 0.25 },
    screen:    { color: 0x050C0E, metalness: 0.10, roughness: 0.12 },
    screenOn:  { color: 0x0B2026, metalness: 0.05, roughness: 0.20, emissive: 0x0A2830, emissiveIntensity: 0.35 },
    deck:      { color: 0x1B2620, metalness: 0.82, roughness: 0.30 },
    keys:      { color: 0x111614, metalness: 0.15, roughness: 0.70 },
    trackpad:  { color: 0x22302A, metalness: 0.70, roughness: 0.22 },
    board:     { color: 0x18376E, metalness: 0.25, roughness: 0.55 },
    chip:      { color: 0x0D1218, metalness: 0.55, roughness: 0.32 },
    ihs:       { color: 0xD0D8DC, metalness: 0.95, roughness: 0.20 },
    gold:      { color: 0xDEB048, metalness: 0.98, roughness: 0.16 },
    cell:      { color: 0x252C32, metalness: 0.25, roughness: 0.60 },
    cellStrip: { color: 0xE8BA5A, metalness: 0.35, roughness: 0.45 },
    nand:      { color: 0x101614, metalness: 0.30, roughness: 0.45 },
    ram:       { color: 0x163424, metalness: 0.30, roughness: 0.50 },
    chassis:   { color: 0x1A2520, metalness: 0.88, roughness: 0.30 },
    copper:    { color: 0xCA6D28, metalness: 0.95, roughness: 0.22 },
    rubber:    { color: 0x0A0F0D, metalness: 0.00, roughness: 0.90 }
  };

  var renderer, scene, camera, root, raf = null;
  var parts = [];
  var lidPivot = null, deckGroup = null;
  var labelHost, canvas, host;
  var _ready = false;
  var progress = 0, from = 0, to = 0, tweenStart = 0, tweening = false;
  var drag = { on: false, x: 0, y: 0, az: 0, el: 0 };
  var idle = 0;

  var clamp = function(v, a, b) { return Math.max(a, Math.min(b, v)); };
  var lerp  = function(a, b, t) { return a + (b - a) * t; };
  var easeOut = function(p) { return 1 - Math.pow(1 - clamp(p, 0, 1), 3); };
  var mkMat = function(spec) { return new THREE.MeshStandardMaterial(spec); };

  function makeBox(w, h, d, spec) {
    return new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mkMat(spec));
  }

  function makeSlab(w, h, d, r, spec) {
    var s = new THREE.Shape();
    var x = -w / 2, z = -d / 2;
    s.moveTo(x, z + r);
    s.lineTo(x, z + d - r); s.quadraticCurveTo(x, z + d, x + r, z + d);
    s.lineTo(x + w - r, z + d); s.quadraticCurveTo(x + w, z + d, x + w, z + d - r);
    s.lineTo(x + w, z + r); s.quadraticCurveTo(x + w, z, x + w - r, z);
    s.lineTo(x + r, z); s.quadraticCurveTo(x, z, x, z + r);
    var bevel = Math.min(0.04, h * 0.18);
    ...
    g.translate(0, h / 2, 0);
    g.computeVertexNormals();
    return new THREE.Mesh(g, mkMat(spec));
  }

  function buildChassis() {
    var g = new THREE.Group();
    // Bottom chassis shell
    var base = makeSlab(22, 0.65, 15, 1.0, M.chassis);
    g.add(base);
    // 4 rubber feet on underside
    [[-9, 6], [9, 6], [-9, -6], [9, -6]].forEach(function(pos) {
      var foot = makeBox(1.2, 0.15, 0.8, M.rubber);
      foot.position.set(pos[0], -0.08, pos[1]);
      g.add(foot);
    });
    // Side USB-C & HDMI ports
    [-1, 1].forEach(function(side) {
      var port1 = makeBox(0.12, 0.22, 0.8, M.gold);
      port1.position.set(side * 11.02, 0.35, -2.0);
      var port2 = makeBox(0.12, 0.22, 0.8, M.gold);
      port2.position.set(side * 11.02, 0.35, -3.5);
      g.add(port1); g.add(port2);
    });
    return g;
  }
  ...
  function buildBattery() {
    var g = new THREE.Group();
    // Dual Li-Poly pouch cells in front half
    [-4.6, 4.6].forEach(function(x) {
      var cell = makeSlab(8.4, 0.50, 6.2, 0.4, M.cell);
      cell.position.set(x, 0, 3.2);
      g.add(cell);
    });
    // Gold regulatory & capacity strip
    var strip = makeBox(17.2, 0.04, 1.0, M.cellStrip);
    strip.position.set(0, 0.52, 3.2);
    g.add(strip);
    // Battery interface connector
    var conn = makeBox(3.0, 0.18, 0.9, M.chip);
    conn.position.set(0, 0.35, 0.0);
    g.add(conn);
    return g;
  }
  ...
  function buildSSD() {
    var g = new THREE.Group();
    var pcb = makeBox(8.6, 0.22, 2.4, M.nand);
    pcb.position.set(-5.0, 0, -1.2);
    g.add(pcb);
    // NAND flash packages
    [-6.2, -4.2].forEach(function(x) {
      var chip = makeBox(1.8, 0.15, 1.8, M.chip);
      chip.position.set(x, 0.16, -1.2);
      g.add(chip);
    });
    // Controller IC
    var ctrl = makeBox(1.5, 0.14, 1.5, M.ihs);
    ctrl.position.set(-2.2, 0.15, -1.2);
    g.add(ctrl);
    // Gold edge contacts
    var edge = makeBox(1.0, 0.08, 2.0, M.gold);
    edge.position.set(-0.6, 0.05, -1.2);
    g.add(edge);
    return g;
  }
  ...
  function buildRAM() {
    var g = new THREE.Group();
    [-0.5, 0.5].forEach(function(offsetZ) {
      var stick = makeBox(8.0, 0.18, 1.6, M.ram);
      stick.position.set(5.0, 0.1 * offsetZ, -1.2 + offsetZ * 1.8);
      g.add(stick);
      for (var j = -3; j <= 3; j += 2) {
        var ic = makeBox(1.2, 0.12, 1.1, M.chip);
        ic.position.set(5.0 + j * 0.9, 0.12 + 0.1 * offsetZ, -1.2 + offsetZ * 1.8);
        g.add(ic);
      }
    });
    return g;
  ...
  function buildMotherboard() {
    var g = new THREE.Group();
    // Mainboard PCB in rear half
    var pcb = makeSlab(20, 0.30, 7.6, 0.6, M.board);
    pcb.position.set(0, 0, -3.2);
    g.add(pcb);
    // CPU with silver Integrated Heat Spreader (IHS)
    var cpu = makeBox(4.2, 0.26, 4.2, M.ihs);
    cpu.position.set(-4.0, 0.22, -3.2);
    g.add(cpu);
    // Dedicated GPU die
    var gpu = makeBox(3.8, 0.22, 3.8, M.chip);
    gpu.position.set(4.0, 0.20, -3.2);
    g.add(gpu);
    // Copper cooling heatpipes
    var pipe = makeBox(17.0, 0.16, 0.55, M.copper);
    pipe.position.set(0, 0.36, -3.2);
    g.add(pipe);
    var pipe2 = makeBox(15.0, 0.16, 0.45, M.copper);
    pipe2.position.set(0, 0.36, -4.0);
    g.add(pipe2);
    // Silver shield cans & chips
    [[-8, -2.2], [-8, -4.4], [8, -2.2], [8, -4.4]].forEach(function(pt) {
      var can = makeBox(1.8, 0.22, 1.6, M.ihs);
      can.position.set(pt[0], 0.18, pt[1]);
      g.add(can);
    });
    return g;
  }
  ...
  function buildKeyboardDeck() {
    var g = new THREE.Group();
    // Top case slab
    var deck = makeSlab(21.6, 0.38, 14.6, 0.9, M.deck);
    g.add(deck);
    // Glass trackpad in front
    var tp = makeBox(7.2, 0.06, 4.8, M.trackpad);
    tp.position.set(0, 0.22, 4.2);
    g.add(tp);
    // Recessed keyboard well
    var well = makeBox(17.8, 0.05, 7.2, { color: 0x111614, metalness: 0.2, roughness: 0.8 });
    well.position.set(0, 0.20, -2.4);
    g.add(well);
    // Chiclet key rows
    for (var r = 0; r < 5; r++) {
      var kRow = makeBox(17.2, 0.12, 1.05, M.keys);
      kRow.position.set(0, 0.26, -5.0 + r * 1.35);
      g.add(kRow);
    }
    // Spacebar
    var sb = makeBox(7.6, 0.12, 1.05, M.keys);
    sb.position.set(0, 0.26, 1.7);
    g.add(sb);
    // Speaker grilles
    [-9.6, 9.6].forEach(function(gx) {
      for (var i = -4; i <= 4; i++) {
        var slot = makeBox(0.12, 0.06, 0.5, { color: 0x0E1411, roughness: 0.9 });
        slot.position.set(gx, 0.22, -2.2 + i * 0.7);
        g.add(slot);
      }
    });
    return g;
  ...
  function buildDisplayLid() {
    var g = new THREE.Group();
    // Outer lid back shell
    var lidShell = makeSlab(22, 0.45, 14.8, 1.0, M.lid);
    lidShell.position.set(0, 0.22, 7.4);
    g.add(lidShell);
    // Metallic emblem logo on the back
    var logo = makeBox(1.6, 0.04, 1.6, M.gold);
    logo.position.set(0, -0.02, 7.4);
    g.add(logo);
    // Inner screen bezel
    var bezel = makeBox(21.4, 0.06, 14.2, M.lid);
    bezel.position.set(0, 0.47, 7.4);
    g.add(bezel);
    // Glossy OLED display panel
    var screen = makeBox(20.4, 0.08, 13.2, M.screenOn);
    screen.position.set(0, 0.51, 7.4);
    g.add(screen);
    // HD Webcam dot at top center bezel
    var cam = makeBox(0.35, 0.08, 0.35, { color: 0x1E2428, metalness: 0.3, roughness: 0.5 });
    cam.position.set(0, 0.52, 14.1);
    g.add(cam);
    return g;
  }
  ...
  function makeLabel(key) {
    ...
    el.className = 'td-label';
    el.style.setProperty('--td-accent', LP_LABEL_ACCENT[key] || '#284139');
    el.innerHTML = '<span class="td-dot"></span><span class="td-stem"></span><span class="td-txt">' +
      '<span class="td-name">' + (LP_LABEL_TEXT[key] || key) + '</span>' +
      '<span class="td-val" data-lppart3d="' + key + '">₹0</span></span>';
    labelHost.appendChild(el);
    return el;
  }
  ...
  function buildScene() {
    ...
    root = new THREE.Group();
    scene.add(root);
    // Scale root so the entire exploded laptop and all badges fit comfortably inside the frame
    root.scale.set(0.80, 0.80, 0.80);

    // Warm, crisp lightbox illumination
    scene.add(new THREE.HemisphereLight(0xFFFBF2, 0xEAE7DF, 0.65));
    var key = new THREE.DirectionalLight(0xFFFAF0, 0.95);
    key.position.set(14, 20, 16);
    scene.add(key);
    var fill = new THREE.DirectionalLight(0x94B8D2, 0.35);
    fill.position.set(-16, 10, 10);
    scene.add(fill);
    var rim = new THREE.PointLight(0xF8D794, 0.45, 120);
    rim.position.set(-10, 16, -14);
    scene.add(rim);

    // 5 base-stack parts (bottom to top): Chassis, Battery, Storage, RAM, Motherboard
    var stackDefs = [
      { key: 'chassis',     build: buildChassis,     seatY: 0.00, apartY: -3.8, anchorZ:  0.0 },
      { key: 'battery',     build: buildBattery,     seatY: 0.25, apartY: -1.4, anchorZ:  3.0 },
      { key: 'storage',     build: buildSSD,         seatY: 0.35, apartY:  0.8, anchorZ: -1.2 },
      { key: 'ram',         build: buildRAM,         seatY: 0.40, apartY:  2.8, anchorZ: -1.2 },
      { key: 'motherboard', build: buildMotherboard, seatY: 0.30, apartY:  5.0, anchorZ: -3.0 }
    ];

    parts = stackDefs.map(function(def) {
      var group = def.build();
      group.position.y = def.seatY;
      root.add(group);
      var anchor = new THREE.Object3D();
      anchor.position.set(12.2, 0.3, def.anchorZ);
      group.add(anchor);
      return {
        group: group, key: def.key,
        seatY: def.seatY, apartY: def.apartY,
        isDisplay: false, anchor: anchor, label: null
      };
    });

    // Keyboard deck (visual layer between motherboard and display)
    deckGroup = buildKeyboardDeck();
    deckGroup.position.y = 0.55;
    root.add(deckGroup);

    // 6. Display Assembly — mounted on hinge pivot at rear edge (Z = -7.4)
    lidPivot = new THREE.Group();
    lidPivot.position.set(0, 0.55, -7.4);
    root.add(lidPivot);

    var lidMesh = buildDisplayLid();
    lidPivot.add(lidMesh);

    // Open laptop angle: ~114° open from base
    lidPivot.rotation.x = -1.98;

    var displayAnchor = new THREE.Object3D();
    displayAnchor.position.set(12.2, 0.5, 7.0);
    lidMesh.add(displayAnchor);

    parts.push({
      group: lidMesh, key: 'display',
      seatY: 0.55, apartY: 10.0,
      isDisplay: true, pivot: lidPivot,
      anchor: displayAnchor, label: null
    });

    // Build HTML badge cards
    labelHost = document.getElementById('lp-teardown-labels');
    if (labelHost) {
      labelHost.innerHTML = '';
      parts.forEach(function(p) {
        p.label = makeLabel(p.key);
      });
    }
  }
  ...
  function placeCamera(t) {
    var w = host ? host.clientWidth : 360;
    var isMobile = w < 600;
    var r  = lerp(isMobile ? 58 : 50, isMobile ? 74 : 64, t);
    var az = lerp(0.35, 0.42, t) + drag.az + Math.sin(idle) * 0.05;
    var el = clamp(lerp(0.44, 0.50, t) + drag.el, 0.08, Math.PI / 2 - 0.08);
    ...
    // Shift lookAt.x to the right as it explodes, smoothly moving the 3D model
    // left to give the right-side callout badges generous space!
    var lookX = lerp(isMobile ? 0.8 : 0.0, isMobile ? 3.6 : 2.4, t);
    var lookY = lerp(3.2, 4.2, t);
    camera.lookAt(lookX, lookY, 0);
  }

  function updateParts(t) {
    parts.forEach(function(p) {
      if (p.isDisplay && p.pivot) {
        // Display lifts vertically and tilts slightly so the face stays in view
        p.pivot.position.y = lerp(0.55, p.apartY, t);
        p.pivot.position.z = lerp(-7.4, -8.2, t);
        p.pivot.rotation.x = lerp(-1.98, -1.86, t);
      } else {
        p.group.position.y = lerp(p.seatY, p.apartY, t);
      }
    });
    if (deckGroup) {
      deckGroup.position.y = lerp(0.55, 7.2, t);
    }
  }

  function placeLabels(t) {
    if (!labelHost || !camera || !host) return;
    ...
    parts.forEach(function(p) {
      ...
      p.anchor.getWorldPosition(v);
      v.project(camera);

      var sx = (v.x * 0.5 + 0.5) * w;
      var sy = (-v.y * 0.5 + 0.5) * h;

      // Mobile optimization: clamp right edge so chips never overflow screen
      if (isMobile) {
        sx = Math.min(sx, w - 105);
      } else {
        sx = Math.min(sx, w - 125);
      }

      // Keep labels safely within vertical canvas bounds
      sy = Math.max(22, Math.min(h - 22, sy));

      var vis = v.z < 1 && sx > 10 && sx < w && sy > 5 && sy < h - 5;
      p.label.style.opacity = vis ? String(clamp((t - 0.25) / 0.35, 0, 1)) : '0';
      p.label.style.transform = 'translate(' + sx.toFixed(1) + 'px,' + sy.toFixed(1) + 'px) translate(0, -50%)';
    });
  }

  function apply(t) {
    updateParts(easeOut(t));
    placeCamera(easeOut(t));
    placeLabels(t);
  }
  ...
  function setValue(key, text) {
    if (!labelHost) return;
    var el = labelHost.querySelector('[data-lppart3d="' + key + '"]');
    if (el) el.textContent = text;
  }
  ...
  function loop(ts) {
    ...
    if (tweening) {
      var elapsed = (ts - tweenStart) / EXPLODE_MS;
      progress = from + (to - from) * clamp(elapsed, 0, 1);
      if (elapsed >= 1) { progress = to; tweening = false; }
    }
    if (!drag.on) idle += 0.002;
    apply(progress);
    renderer.render(scene, camera);
  }
  ...
  function init(hostEl) {
    ...
    host = hostEl;
    canvas = document.getElementById('lp-teardown-canvas');
    ...
    window.addEventListener('mousemove', function(e) {
      if (!drag.on) return;
      drag.az += (e.clientX - drag.x) * 0.007;
      drag.el = clamp(drag.el - (e.clientY - drag.y) * 0.007, -0.4, 0.5);
      drag.x = e.clientX; drag.y = e.clientY;
    }, { passive: true });
    window.addEventListener('mouseup', function() { drag.on = false; }, { passive: true });
    ...
    canvas.addEventListener('touchmove', function(e) {
      if (!drag.on || e.touches.length !== 1) return;
      drag.az += (e.touches[0].clientX - drag.x) * 0.007;
      drag.el = clamp(drag.el - (e.touches[0].clientY - drag.y) * 0.007, -0.4, 0.5);
      drag.x = e.touches[0].clientX;
      drag.y = e.touches[0].clientY;
    }, { passive: true });
    canvas.addEventListener('touchend', function() { drag.on = false; }, { passive: true });
  ...
  function explode()  { from = progress; to = 1; tweenStart = performance.now(); tweening = true; }
  function assemble() { from = progress; to = 0; tweenStart = performance.now(); tweening = true; }
  function destroy()  {
    if (raf) cancelAnimationFrame(raf); raf = null;
    if (renderer) { renderer.dispose(); renderer = null; }
    _ready = false; parts = []; lidPivot = null; deckGroup = null;
  }

  return {
    get ready()    { return _ready; },
    get exploded() { return progress > 0.5; },
    get durationMs() { return EXPLODE_MS; },
    init: init, explode: explode, assemble: assemble, destroy: destroy, setValue: setValue
  };
})();


function initLaptopTeardown3D() {
  if (!window.THREE) return;
  var hostEl = document.getElementById('lp-disassembly-stage');
  if (!hostEl) return;
  var tdDiv = document.getElementById('lp-teardown-3d');
  if (!tdDiv) return;
  var ok = LaptopTeardown3D.init(hostEl);
  if (ok) {
    tdDiv.classList.remove('hidden');
    var svgShell = document.getElementById('lp-svg-shell');
    if (svgShell) svgShell.classList.add('hidden');
    hostEl.classList.remove('viewport-3d');
    hostEl.classList.add('stage-light');
  }
}
...
// ─── FIREBASE CLOUD DATABASE CART PERSISTENCE ───
window._pendingCartSync = false;
...
async function saveCartToFirebase() {
  const uid = getActiveUID();
  console.log("[Firebase] Saving cart items for UID:", uid, S.cartItems);

  if (!window.db) {
    console.warn("[Firebase] Firestore not initialized yet. Cart will sync once connection establishes.");
    window._pendingCartSync = true;
    return;
  }

  try {
    // 1. Primary: Save to users collection document as requested
    await window.db.collection('users').doc(uid).set({
      cartItems: S.cartItems,
      cartCount: S.cartItems.length,
      cartSubtotal: S.cartItems.reduce((a, b) => a + b.price, 0),
      updatedAt: new Date().toISOString()
    }, { merge: true });

    // 2. Also save to dedicated 'carts' collection for easy database inspection
    await window.db.collection('carts').doc(uid).set({
      uid: uid,
      cartItems: S.cartItems,
      cartCount: S.cartItems.length,
    ...
    console.log("✓ [Firebase] Cart successfully saved to Firestore Database for UID:", uid, S.cartItems);
  } catch (err) {
    console.error("[Firebase] Error saving cart items to Firestore:", err);
  }
}

async function loadAllUserDataFromFirebase(uid) {
  if (!window.db || !uid) return;
  try {
    const userDoc = await window.db.collection('users').doc(uid).get();
    if (userDoc.exists) {
      const data = userDoc.data();
      console.log("✓ [Firebase] Hydrating complete user state from Firestore for UID:", uid, data);

      // 1. Cart Items
      if (Array.isArray(data.cartItems)) {
        S.cartItems = data.cartItems;
        window.cartItems = S.cartItems;
        updateCartUI(false);
      }

      // 2. Saved Quotes
      if (Array.isArray(data.quotes || data.savedQuotes)) {
        window.DB.setLocal('quotes', data.quotes || data.savedQuotes);
      }

      // 3. Orders
      if (Array.isArray(data.orders)) {
        window.DB.setLocal('orders', data.orders);
      }

      // 4. Addresses
      if (Array.isArray(data.addresses)) {
        window.DB.setLocal('addresses', data.addresses);
      }

      // 5. User Profile
      if (data.user || data.profile) {
        const u = data.user || data.profile;
        window.DB.setLocal('user', u);
        if (u.name) {
          const el = document.getElementById('prof-name');
          if (el) el.textContent = u.name;
          const authLbl = document.getElementById('auth-label');
          if (authLbl) authLbl.textContent = u.name.split(' ')[0];
        }
        if (u.city) {
        ...
        if (u.photoURL) {
          const pImg = document.getElementById('prof-avatar-img');
          if (pImg) { pImg.src = u.photoURL; pImg.classList.remove('hidden'); }
          const pIcon = document.getElementById('prof-avatar-icon');
          if (pIcon) pIcon.classList.add('hidden');
          const nImg = document.getElementById('nav-user-photo');
          if (nImg) { nImg.src = u.photoURL; nImg.classList.remove('hidden'); }
          const nIcon = document.getElementById('nav-google-icon');
          if (nIcon) nIcon.classList.add('hidden');
        }
      }

      // 6. Messages
      if (Array.isArray(data.messages || data.advisorChat)) {
        window.DB.setLocal('messages', data.messages || data.advisorChat);
      }

      renderDatabaseUI();
    } else {
      // First visit: Initialize Firestore document with existing local baseline
      const initialPayload = {
        uid: uid,
        cartItems: S.cartItems || [],
        quotes: window.DB.get('quotes', []),
        orders: window.DB.get('orders', []),
        addresses: window.DB.get('addresses', []),
        user: window.DB.get('user', { name: 'Guest User', city: 'Bhatpara, WB' }),
        createdAt: new Date().toISOString()
      };
      await window.db.collection('users').doc(uid).set(initialPayload, { merge: true });
      console.log("✓ [Firebase] Initialized new Firestore document for UID:", uid);
    }
  } catch (err) {
    console.warn("[Firebase] Could not fetch complete user data from Firestore:", err);
  }
}
  ...
  try {
    ...
    if (userDoc.exists) {
      ...
      if (Array.isArray(data.cartItems)) {
        ...
        updateCartUI(false);
        loaded = true;
        console.log("✓ [Firebase] Loaded", S.cartItems.length, "cart items from users collection for UID:", uid);
      }
      if (Array.isArray(data.savedQuotes)) window.DB.set('quotes', data.savedQuotes);
      if (Array.isArray(data.orders)) window.DB.set('orders', data.orders);
      if (Array.isArray(data.addresses)) window.DB.set('addresses', data.addresses);
      renderDatabaseUI();
    }

    if (!loaded) {
      const cartDoc = await window.db.collection('carts').doc(uid).get();
      if (cartDoc.exists && Array.isArray(cartDoc.data().cartItems)) {
        S.cartItems = cartDoc.data().cartItems;
        window.cartItems = S.cartItems;
        updateCartUI(false);
        console.log("✓ [Firebase] Loaded", S.cartItems.length, "cart items from carts collection for UID:", uid);
      } else {
        await window.db.collection('users').doc(uid).set({
          uid: uid,
          cartItems: [],
    ...
    console.warn("[Firebase] Could not fetch cart from Firestore:", err);
  }
}
...
function addToCart(id, quiet) {
  ...
  window.cartItems = S.cartItems;
  updateCartUI(true);
  if (!quiet) toast(`${it.title} added to cart & saved to Firebase (+₹5 Greener India Pledge)`);
  saveCartToFirebase();
  window.DB.set('cartItems', S.cartItems);
}
...
function wizFocus() {
  const first = document.querySelector('.wiz-step.on .wiz-input');
...
function wizClearError() {
  ...
  document.querySelectorAll('.wiz-input.bad')
...
function wizPlace() {
  ...
  if (wizEl('wiz-save').checked) {
    window.DB.set('ship', ship);
    // Directly sync and store user shipping details into Cloud Firestore
    if (window.currentUserUID && window.db) {
      window.db.collection('users').doc(window.currentUserUID).set({
        ...
        customerAddress: `${ship.addr}, ${ship.city} ${ship.pin}`,
        lastUpdated: new Date().toISOString()
      }, { merge: true }).catch(err => console.warn("Firestore shipping details sync error:", err));
    }
  }
...
function setLoc(name, label) {
  ...
  const user = window.DB.get('user', {});
  user.city = name;
  window.DB.set('user', user);
  closeModal('loc-modal');
  toast('Location Hub updated & saved to Firebase: ' + name);
}
...
function previewAvatar(event) {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      const navImg = document.getElementById('nav-user-photo');
      if (navImg) { navImg.src = dataUrl; navImg.classList.remove('hidden'); }
      const navIcon = document.getElementById('nav-google-icon');
      if (navIcon) navIcon.classList.add('hidden');

      const user = window.DB.get('user', {});
      user.photoURL = dataUrl;
      window.DB.set('user', user);
      toast('Profile photo updated & saved to Firebase');
    };
...
(function initScrollUI() {
  ...
  const measure = () => {
    ...
    /* ── Reads first, then writes ──────────────────────────────────────
       These were interleaved: the progress bar written, then the section
       rectangles read, then the ambience custom properties written, then
       the rail's rectangles read. Every read that follows a write makes
       the browser flush style and layout synchronously to answer it, so a
       single scroll frame paid for three full layouts. Identical work,
       reordered — everything measured while the tree is still clean, and
       nothing written until nothing is left to ask. */
    const max     = document.documentElement.scrollHeight - window.innerHeight;
    const scrollY = Math.max(0, window.scrollY);
    const line    = window.innerHeight * 0.35;
    const onHome  = S.page === 'home';

    // Where the sight line falls between two consecutive sections, 0..1.
    const tops = onHome ? AMBIENCE.map(a => {
      const el = document.getElementById(a.id);
      return el ? el.getBoundingClientRect().top - line : Infinity;
    }) : null;

    let current = null;
    if (onHome && dots.length) {
      current = dots[0];
      for (let k = 0; k < dots.length; k++) {
        const sec = document.getElementById(dots[k].dataset.sec);
        if (sec && sec.getBoundingClientRect().top <= line) current = dots[k];
      }
      /* The last section is shorter than the sight line is deep, so the page
         ...
         shows from 1280px up, so tall windows are the common case, not the
         edge. At the foot of the document there is nothing below to point at,
         so the last dot is the answer whatever the sight line says. */
      if (max > 0 && scrollY >= max - 2) current = dots[dots.length - 1];
    }

    /* ── nothing below this line reads layout ── */
    ...
    if (bars.length) {
      if (Math.abs(scrollY - lastY) > 6) {
        const hide = scrollY > lastY && scrollY > 120;
        bars.forEach(b => b.classList.toggle('nav-hidden', hide));
        lastY = scrollY;
      }
      if (scrollY <= 120) bars.forEach(b => b.classList.remove('nav-hidden'));
    }
    ...
    if (current) dots.forEach(d => d.classList.toggle('active', d === current));
  };
...
(function initGlyphString() {
  if (document.documentElement.classList.contains('lowfi')) return;
  ...
  let raf = 0, running = true, reduced = false, force = false;
  let lastTick = -1e9;
  ...
  function drawAll(t, full) {
    ...
    for (let i = 0; i < veils.length; i++) {
      ...
      /* Scrolled past: hand the backing store back rather than hold a canvas
         the size of the canopy for a band nobody can see. A canopy-width
         veil is megabytes, and it is rebuilt below in one assignment on the
         first frame it is wanted again. */
      if (vB <= vT) {
        if (v.canvas.width) { v.canvas.width = v.canvas.height = 0; }
        continue;
      }
      if (!v.canvas.width) {
        v.canvas.width  = Math.round(v.w * dpr);
        v.canvas.height = Math.round(v.h * dpr);
      }
  ...
  function frame(ts) {
    /* The ribbon is a scroll instrument: its marks advance only while the
       page is actually moving under them, and the chain ends the moment
       scrolling stops rather than running on for as long as somebody is at
       the machine. A cable that holds still when the page holds still is
       also the honest reading of it — it is drawn as something the page
       moves past, not something with a life of its own.

       `force` is the single pass owed to geometry that changed without
       anyone scrolling: a resize, a view switch, the fonts landing, coming
       back to the tab. Without it those would leave the band stale until
       the next scroll. */
    const moving = Activity.scrolling();
    if (!running || (!moving && !force)) { raf = 0; return; }
    raf = requestAnimationFrame(frame);
    const scrollY = window.scrollY || window.pageYOffset || 0;
  ...
  window.addEventListener('resize', () => {
    clearTimeout(rt);
    rt = setTimeout(() => { layout(); sig = ''; wake(); }, 140);
  });

  /* The sprites bake whatever font resolved when they were cut, so recut once
     JetBrains Mono lands and force a repaint. */
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(() => { buildSprites(); anchor = -1e9; wake(); }).catch(() => {});
  }

  /* wake() is for geometry: it owes the band one drawn pass. Scrolling
     starting just needs the chain running — frame() sees `moving` for
     itself and keeps going until the page stops. */
  function wake() { force = true; if (!raf && running) raf = requestAnimationFrame(frame); }
  /* Anything that moves the canopies without scrolling the page asks for a
     pass through here — see nav(). */
  window.glyphRepaint = wake;
  Activity.onScrollStart(() => { if (!raf && running) raf = requestAnimationFrame(frame); });

  document.addEventListener('visibilitychange', () => {
    running = !document.hidden;
    if (running) { anchor = -1e9; wake(); }
    else {
      /* Backgrounded: drop every pixel the ribbon is holding. The band alone
         is a viewport-and-a-bit of canvas. anchor is reset above on the way
         back, which forces the full redraw that rebuilds them. */
      band.width = band.height = 0;
      for (let i = 0; i < veils.length; i++) {
        if (veils[i].canvas.width) { veils[i].canvas.width = veils[i].canvas.height = 0; }
      }
    }
  });
...
(async function initFirebase() {
  ...
  let getFirestore, doc, getDoc, setDoc, updateDoc, onSnapshot;
  ...
  // Expose window.db compatible with db.collection(name).doc(id).get() / set()
  window.db = {
    instance: firestore,
    collection: (colName) => ({
      doc: (docId) => ({
        ...
        set: async (data, options = {}) => {
          return await setDoc(doc(firestore, colName, docId), data, options);
        },
        update: async (data) => {
          return await updateDoc(doc(firestore, colName, docId), data);
        },
        onSnapshot: (callback) => {
          return onSnapshot(doc(firestore, colName, docId), (snap) => {
            callback({
              exists: snap.exists(),
              data: () => snap.data() || {},
              id: snap.id
  ...
  console.log("✓ Firebase Auth & Firestore successfully connected to project: resell-123");

  // Flush any pending changes that occurred before Firebase initialized
  const currentInitialUid = getActiveUID();
  if (window._pendingFirebaseChanges && Object.keys(window._pendingFirebaseChanges).length > 0) {
    for (const [k, v] of Object.entries(window._pendingFirebaseChanges)) {
      window.syncChangeToFirebase(k, v);
    }
    window._pendingFirebaseChanges = {};
  }
  if (window._pendingCartSync || S.cartItems.length > 0) {
    saveCartToFirebase();
  }

  // Hydrate all user state (cart, quotes, orders, addresses, profile) from Firebase
  loadAllUserDataFromFirebase(currentInitialUid);
  ...
  window.FirebaseAuth = {
    ...
    signInWithGoogle: async () => {
      try {
        ...
        // Sync current cart to newly logged in user
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      window.currentUserUID = user.uid;
      window.handleFirebaseUserLogin(user);
      await loadAllUserDataFromFirebase(user.uid);
      if (S.cartItems.length > 0) {
        await saveCartToFirebase();
        window.DB.set('cartItems', S.cartItems);
      }
      ...
      // Load device-level all user data if exists
      const guestUid = getActiveUID();
      await loadAllUserDataFromFirebase(guestUid);
    }
  });
})();
