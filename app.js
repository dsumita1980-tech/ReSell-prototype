...
window.DB = {
  ...
  set: (k, v) => {
    ...
    // Requirement 2: Real-time Cloud Firestore Data Syncing
    if (window.currentUserUID && window.db) {
      ...
      }, { merge: true }).catch(e => console.warn("Firestore auto-sync error:", e));
    }
  }
};
...
function resetSellAndDisassembleToDefault() {
  // Reset Whole Phone Preview & Exploded 3D Grid
  const wholePhone = document.getElementById('whole-phone-preview');
  const explodedGrid = document.getElementById('exploded-3d-grid');
  if (wholePhone) {
    wholePhone.classList.remove('hidden', 'phone-collapsing');
  }
  if (explodedGrid) {
    explodedGrid.classList.add('hidden');
    explodedGrid.classList.remove('flex', 'animating');
  }
...
function nav(p) {
  S.page = p;
  resetSellAndDisassembleToDefault(); // Reset teardown stage on enter/exit
...
function computeQuote() {
  ...
  const STRICT_DEPLETION_FACTOR = 0.70;
  const aged = (base) => base * Math.pow(1 - bom.decay, ageMonths) * STRICT_DEPLETION_FACTOR;
...
function renderTrendChartRealistic(brand, mult, ageMonths) {
  ...
  const simulateCurve = (base, volatility) => {
    const strictBase = base * 0.70; // 30% stricter price depletion
    return [
      Math.round(strictBase * mult * 1.08),
      Math.round(strictBase * mult * 1.06 + Math.sin(1) * volatility),
      Math.round(strictBase * mult * 1.03),
      Math.round(strictBase * mult * 1.05 + Math.cos(2) * volatility),
      Math.round(strictBase * mult * 1.01),
      Math.round(strictBase * mult * 0.98),
      Math.round(strictBase * mult * 0.96 - volatility),
      Math.round(strictBase * mult * 0.94),
      Math.round(strictBase * mult * 0.95 + volatility * 0.5),
      Math.round(strictBase * mult * 0.91),
      Math.round(strictBase * mult * 0.88),
      Math.round(strictBase * mult * 0.85)
    ];
  };
...
(function initGlyphString() {
  return; // Disabled: background is completely static
...
(function initCursorGrid() {
  return; // Disabled background constant canvas animation
  return; // Disabled background constant canvas animation for lightweight performance
...
(function initCursorDot() {
  return; // Disabled custom cursor dot
  return; // Disabled custom cursor dot to keep normal browser cursor
...
(async function initFirebase() {
  ...
  // Expose Firestore instance with db.collection(col).doc(id).get() and set()
  window.db = {
    instance: firestore,
    collection: (colName) => ({
      doc: (docId) => ({
  ...
  // Synchronize Firestore user document (Requirements 1, 2, 3)
  async function syncAndLoadFirestoreUser(user) {
    ...
    try {
      ...
      if (!userSnap.exists) {
        ...
        const newProfile = {
          uid: user.uid,
          email: user.email || '',
          displayName: user.displayName || 'ReSell Member',
          photoURL: user.photoURL || '',
          cartItems: S.cartItems || [],
          savedQuotes: window.DB.get('quotes', []),
        ...
        console.log("Firestore: Profile created for new user:", user.uid);
      } else {
        // Requirement 3: Data Retrieval
        const cloudData = userSnap.data();
        console.log("Firestore: Cloud data retrieved for user:", user.uid, cloudData);

        if (Array.isArray(cloudData.cartItems)) {
          S.cartItems = cloudData.cartItems;
          if (typeof renderCart === 'function') renderCart();
        }
        if (Array.isArray(cloudData.savedQuotes)) {
          window.DB.set('quotes', cloudData.savedQuotes);
        }
        if (Array.isArray(cloudData.orders)) {
          window.DB.set('orders', cloudData.orders);
        }
        if (Array.isArray(cloudData.addresses)) {
          window.DB.set('addresses', cloudData.addresses);
        }

        // Update basic info in Firestore with merge
        await userRef.set({
          email: user.email || '',
        ...
        if (typeof renderDatabaseUI === 'function') renderDatabaseUI();
      }

      // Realtime listener across browser tabs
      if (!window._firestoreUnsub) {
        window._firestoreUnsub = userRef.onSnapshot((snap) => {
          if (snap.exists) {
            const liveData = snap.data();
            if (Array.isArray(liveData.cartItems) && JSON.stringify(liveData.cartItems) !== JSON.stringify(S.cartItems)) {
              S.cartItems = liveData.cartItems;
              if (typeof renderCart === 'function') renderCart();
            }
          }
        });
  ...
  window.FirebaseAuth = {
    ...
    signInWithGoogle: async () => {
      try {
        ...
        window.handleFirebaseUserLogin(user);
        await syncAndLoadFirestoreUser(user);
        return user;
      } catch (error) {
    ...
    signOutUser: async () => {
      try {
        if (window._firestoreUnsub) {
          window._firestoreUnsub();
          window._firestoreUnsub = null;
        }
